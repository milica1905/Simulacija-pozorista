...
public class Kontroler_Radnik_Dodaje_Izvodjenje implements Initializable{
	  ...
	  void potvrdaChoiceBoxPredstave(ActionEvent event) {
		  if(choiceBoxPredstave.getValue() == null) {
			  ...
			  for(Izvodjenje_predstave ip : Izvodjenje_predstave.getLista_izvodjenja_predstave()) {
				  if(ip.getPredstava_id() == choiceBoxPredstave.getValue().getId() && ip.getPozoriste_id() == Kontroler_Nakon_prijave_Radnika.vratiPozoristeID()) {
					  ispisPorukuOCijeni.setVisible(true);
					  ispisPorukuOCijeni.setText(choiceBoxPredstave.getValue().getNaziv() + " se izvodi vec \nu pozoristu " + 
					  Pozoriste.vratiPozoristeNaOsnovuId(Kontroler_Nakon_prijave_Radnika.vratiPozoristeID()).getNaziv() + "\nTe cijena izvodjenja predstave je " + 
	   ...
	   void dodajIzvodjenje(ActionEvent event) {
		   ...
		   if(datum.getValue() == null && hh.getText().equals("") && ss.getText().equals("") && mm.getText().equals("")) {
		   	...
		   	String s = "Rezime informacija.Radnice, " + Kontroler_Radnik_prijava.imeIPrezimeRadnika() + " u pozoristu " + Pozoriste.vratiPozoristeNaOsnovuId(Kontroler_Nakon_prijave_Radnika.vratiPozoristeID()).getNaziv() + " odabrali ste sljedece:\n";
		   	s += "Predstava " + Predstava.vratiNazivPredstave(choiceBoxPredstave.getValue().getId()) + ".\n";
		   	s += "Cijena: " + cijena1;
			...
			parsed = df.parse(datumivrijeme);
			 ts= new Timestamp(parsed.getTime());
				...
				connect = Konekcija.getConnection();
			   	try {
			   		if(Izvodjenje_predstave.daLiPostojiIzvodjenje( choiceBoxPredstave.getValue().getId(), Kontroler_Nakon_prijave_Radnika.vratiPozoristeID(), cijena1, ts)){
			   		   Alert alert0 = new Alert(Alert.AlertType.WARNING);
			    	 	alert0.setTitle("UPOZORENJE");
	    				...
	    				statement.setInt(2, Kontroler_Nakon_prijave_Radnika.vratiPozoristeID());
