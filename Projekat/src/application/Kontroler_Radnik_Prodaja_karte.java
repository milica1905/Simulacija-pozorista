...
public class Kontroler_Radnik_Prodaja_karte implements Initializable{
    @FXML
    ...
    void switchToRadNaBlagajni(ActionEvent event) {
		...
		try {
			...
			stage.setTitle("Radnice " + Kontroler_Radnik_prijava.imeIPrezimeRadnika()  + ", manipulisite prodajom/rezervacijama karata!");
    ...
    void potvrdaPredstava(ActionEvent event) {
    	if(cbListaPredstava.getValue()==null) {
    		...
    		for(Izvodjenje_predstave ip : Izvodjenje_predstave.getLista_izvodjenja_predstave()) {
    			if(ip.getPozoriste_id() == Kontroler_Nakon_prijave_Radnika.vratiPozoristeID() && ip.getPredstava_id() == predstava.getId()) {
    ...
    void prikaziBrojSlobodnihKarata(ActionEvent event) {
    	if(choiceBoxTermini.getValue() == null) {
    		...
    		if(checkBoxBrojKarata.isSelected()) {
    			...
    			int brojSjedistaUPoz = Pozoriste.vratiPozoristeNaOsnovuId(Kontroler_Nakon_prijave_Radnika.vratiPozoristeID()).getBroj_sjedista();
    			Izvodjenje_predstave ipSelected = choiceBoxTermini.getValue();
    		
    			for(Karta k : Karta.getLista_karata()) {
    				if(k.getIzvodjenje_predstave() == ipSelected.getId()) {
    			...
    			if(brojSjedistaUPoz == 0) {
    				...
    				nemaslkarata.setText("Nažalost, nema slobodnih mjesta u terminu: " + ipSelected.toString());
    			}
    			//checkBoxBrojKarata.setSelected(false);
    ...
    void prikazRezervacija(ActionEvent event) {
    	if(choiceBoxTermini.getValue() == null) {
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	alert0.setTitle("UPOZORENJE");
    	 	alert0.setContentText("NEOPHODNO JE DA IZABERETE TERMIN IZVODJENJA, KOJI VAM NAJVISE ODGOVARA");
    	 	alert0.show();
    	}
    	else {
    		...
    		for(Karta k : Karta.getLista_karata()) {
    			if(k.getIzvodjenje_predstave() == choiceBoxTermini.getValue().getId()) {
    ...
    void prodajaKarte(ActionEvent event) {
    	...
    	else if(Pozoriste.dajBrojSlobodnihSjedista(Pozoriste.vratiPozoristeNaOsnovuId(Kontroler_Nakon_prijave_Radnika.vratiPozoristeID()), choiceBoxTermini.getValue()) < Integer.parseInt(textFieldBrojKarata.getText())) {
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	alert0.setTitle("UPOZORENJE");
    	 	String s = "Slobodan broj sjedista je " +Pozoriste.dajBrojSlobodnihSjedista(Pozoriste.vratiPozoristeNaOsnovuId(Kontroler_Nakon_prijave_Radnika.vratiPozoristeID()), choiceBoxTermini.getValue()) +", a vama je potrebno " + textFieldBrojKarata.getText() + 
    		...
    		if(imaNalogID.isSelected()) {
    			...
    			for(Posjetilac_pozorista pp : Posjetilac_pozorista.getLista_posjetilaca_pozorista()) {
    				if(pp.getKorisnicko_ime().equals(korisnickoImeImatf.getText())) {
    					...
    					if(!pp.getLozinka().equals(Posjetilac_pozorista.getMd5(lozinkaImatf.getText()))) {
    						...
    						Alert alert0 = new Alert(Alert.AlertType.WARNING);
        		    	 	alert0.setTitle("UPOZORENJE");
        		    	 	alert0.setContentText("Neispravna lozinka za " + korisnickoImeImatf.getText() + " .Pokusajte ponovo");
        		    	 	alert0.show();
    					}
    					else {
    						uslov1=true;
    			...
    			if(postojiLiPosjetilac && uslov && uslov1) {
						...
						//REZIME INFORMACIJA
					 	...
					 	s+= "Pozoriste: " + Pozoriste.vratiPozoristeNaOsnovuId(Kontroler_Nakon_prijave_Radnika.vratiPozoristeID());
					 	s+= "Predstava" + cbListaPredstava.getValue();
					 	s += "Termin izvodjenja predstave: " + choiceBoxTermini.getValue()+"\n";
					 	s += "Broj karata za rezervaciju: " + textFieldBrojKarata.getText();
					 	alert0.setContentText(s);
					 	...
					 	//	for(int i =0; i<Integer.parseInt(textFieldBrojKarata.getText());i++) {
					 			try {
				    				...
				    				statement.setInt(4, Integer.parseInt(textFieldBrojKarata.getText()));
				    				//statement.setInt(4, Karta.sljedeciSlobodanBroj( choiceBoxTermini.getValue().getId(), Pozoriste.vratiPozoristeNaOsnovuId(Kontroler_Nakon_prijave_Radnika.vratiPozoristeID()).getBroj_sjedista()));
				 	...
				 	if(result.get()==ButtonType.OK) {
				 		...
				 		//for(int i =0; i<Integer.parseInt(textFieldBrojKarata.getText());i++) {
				 			try {
			    				...
			    				statement.setInt(1, choiceBoxTermini.getValue().getId());
			    				statement.setInt(2, 1);
			    				statement.setInt(3, Posjetilac_pozorista.vratiPosjetiocaNaOsnovuStringa(korisnickoImeNematf.getText()).getId());
			    				statement.setInt(4, Integer.parseInt(textFieldBrojKarata.getText()));
			    				//statement.setInt(4, Karta.sljedeciSlobodanBroj( choiceBoxTermini.getValue().getId(), Pozoriste.vratiPozoristeNaOsnovuId(Kontroler_Nakon_prijave_Radnika.vratiPozoristeID()).getBroj_sjedista()));
			    				statement.executeUpdate();
